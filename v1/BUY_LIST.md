...
| Hobbywing XRotor 40A Opto ESC ×6 (individual, not 4-in-1/Pro/32) | ReadyMadeRC | $19.99 ea ($119.94) | not captured | https://www.readymaderc.com/products/details/85497-hobbywing-xrotor-40a-2-6s-opto-esc-for-multirotor | Relinked 2026-08-03: the HobbywingDirect listing had a preorder/availability issue. Same Hobbywing XRotor 40A individual ESC family as before — 2-6S, 40A cont/60A peak, rated for 550-650mm multirotors — sourced from RMRC, where the identical listing is already used in this doc's Budget-trim tier (confirmed in stock at time of check). Opto (no onboard BEC) — FC power comes from a separate power module/PDB in this build regardless, so that's not a functional change. ~$12 more total across 6 units, negligible against this tier's ~$600+ headroom. Amazon alternative if you want Prime shipping: "Xiangtat Xrotor 40A APAC" ESC (ASIN B00VSQPQF4), same 2-6S/40A spec, found via search but not fetch-verified (Amazon blocks automated fetch). |
...
| Holybro Micro M10 GPS w/ IST8310 compass | Holybro (official store) | $27.99 | 14g | https://holybro.com/products/micro-m10-gps | Swapped in 2026-08-03: the originally-listed M9N Micro was out of stock at RaceDayQuads. Same JST-GH-6P connector and near-identical footprint as the M9N it replaces, so no wiring/mount change. u-blox M10 (2.0m CEP vs M9N's 1.5m) — irrelevant here, project only needs 1-3m accuracy. Also cheaper ($19 under the row's old price). |
...
| IMAX B6AC 80W 6A balance charger, built-in AC adapter, 1-6S LiPo/Li-ion/LiFe | Amazon | ~$25-30 (price not confirmed — Amazon blocked automated fetch, same as before) | — | https://www.amazon.com/Balance-Charger-Discharger-Quadcopter-Accessory/dp/B0GWMZ5FCD | Relinked 2026-08-03: the old ASIN (B01NB9A36R, HOBBYMATE-branded) is dead — returns an error page. This is the same class of part (IMAX B6AC-style clone, AC/DC, cell-balancing, covers the project's 1-6S/6S need), found live via search but not directly fetch-verified (Amazon blocks automated fetch). Verify live price/stock before ordering. |
